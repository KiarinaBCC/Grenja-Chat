...
function App() {
  ...
  useEffect(() => {
    ...
    peer.on('connection', (conn) => {
      ...
      conn.on('close', () => {
        ...
        setMessages(prev => [...prev, { sender: 'system', text: 'Connection closed', timestamp: formatTimestamp() }]);
  ...
  const connectToPeer = async () => {
    if (!remotePeerId) {
      showNotification('Please enter Peer ID');
      return;
    }
  ...
  const sendMessage = async () => {
    if (message && connRef.current && cryptoKey.current) {
      ...
      showNotification('You need a connection and encryption key to send messages');
    }
  };
  ...
  return (
    <div className="min-h-screen gradient-bg">
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        {/* Connect section (before connection) */}
        {connectionStatus !== 'Connected' && (
          <div className="bg-white shadow-xl p-6 mb-6">
